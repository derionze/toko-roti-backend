"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ProdukService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const produk_entity_1 = require("./entities/produk.entity");
const typeorm_3 = require("typeorm");
let ProdukService = class ProdukService {
    produkRepo;
    constructor(produkRepo) {
        this.produkRepo = produkRepo;
    }
    async findByQuery(searchItem, category) {
        const where = {};
        if (searchItem) {
            where.namaProduk = (0, typeorm_3.ILike)(`%${searchItem}%`);
        }
        if (category) {
            where.kategori = (0, typeorm_3.ILike)(`%${category}%`);
        }
        return this.produkRepo.find({ where });
    }
    async create(data) {
        const produk = this.produkRepo.create(data);
        return this.produkRepo.save(produk);
    }
    async findAll() {
        return this.produkRepo.find();
    }
    async findOne(id) {
        const produk = await this.produkRepo.findOneBy({ id });
        if (!produk) {
            throw new common_1.NotFoundException(`Produk dengan id ${id} tidak ditemukan`);
        }
        return produk;
    }
    async update(id, data) {
        const produk = await this.findOne(id);
        Object.assign(produk, data);
        return this.produkRepo.save(produk);
    }
    async remove(id) {
        const result = await this.produkRepo.delete(id);
        if (result.affected === 0) {
            throw new common_1.NotFoundException(`Produk dengan id ${id} tidak ditemukan`);
        }
    }
};
exports.ProdukService = ProdukService;
exports.ProdukService = ProdukService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(produk_entity_1.ProdukEntity)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], ProdukService);
//# sourceMappingURL=produk.service.js.map